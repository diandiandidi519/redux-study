...
function combineReducers(reducers) {
  ...
  return function combination(state, action) {
    ...
    for (var _i = 0; _i < finalReducerKeys.length; _i++) {
      //老的reducer对应的key
      var _key = finalReducerKeys[_i]; // 老的reducer函数

      var reducer = finalReducers[_key]; // 当前key对应的的state

      var previousStateForKey = state[_key]; // 获取触发action以后对应的state
      ...
      nextState[_key] = nextStateForKey; // 对比前后的状态看下有没有改变，浅比较并且是全等
